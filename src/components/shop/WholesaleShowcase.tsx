...
import { X, Info, Layers, ArrowRight, MessageCircle, Package, ShoppingCart, Search } from 'lucide-react';
...
interface WholesaleShowcaseProps {
    products: Product[];
    categories: { id: string; name: string }[];
    carouselSlides: CarouselSlide[];
}

export default function WholesaleShowcase({ products, categories, carouselSlides }: WholesaleShowcaseProps) {
    ...
    return (
        <div className="min-h-screen bg-neutral-950 text-white selection:bg-primary-500/30">
            ...
            {/* Benefits Banner */}
            <div className="bg-neutral-900/50 border-y border-white/5 py-8">
                <div className="max-w-7xl mx-auto px-4">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
                        {[
                            { title: 'Bulk Pricing', desc: 'Discounts available on large orders' },
                            { title: 'Priority Service', desc: 'Dedicated support for wholesale clients' },
                            { title: 'Fast Fulfillment', desc: 'Quick dispatch for bulk orders' }
                        ].map((benefit, idx) => (
                ...
                <div className="max-w-7xl mx-auto px-4">
                    <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
                        ...
                        <div className="relative w-full md:w-72 group">
                            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-neutral-500 group-focus-within:text-primary-500 transition-colors" />
                            <input
                                type="text"
                                placeholder="Search products..."
                                value={searchQuery}
                                onChange={(e) => setSearchQuery(e.target.value)}
                                className="w-full bg-neutral-900 border border-white/10 rounded-full py-2 pl-10 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500/50 transition-all text-white placeholder:text-neutral-600"
                            />
                            {searchQuery && (
                                <button
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {filteredProducts.map((product) => (
                            <div
                                ...
                                className="group relative bg-neutral-900/50 rounded-2xl overflow-hidden cursor-pointer border border-white/5 hover:border-primary-500/50 transition-all duration-500 hover:shadow-[0_0_30px_rgba(249,115,22,0.1)] hover:-translate-y-2"
                            >
                                {/* Image */}
                                <div className="aspect-[4/5] relative overflow-hidden bg-neutral-800">
                                    ...
                                    <div className="absolute bottom-6 right-6 translate-y-10 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-500">
                                        <button className="h-12 w-12 rounded-full bg-white text-black flex items-center justify-center shadow-lg hover:bg-primary-400 transition-colors">
                                ...
                                <div className="absolute bottom-6 left-6 right-16">
                                    <p className="text-primary-400 text-xs font-bold uppercase tracking-wider mb-2">
                                        {product.category}
                                    </p>
                                    ...
                                    <p className="text-sm font-medium text-neutral-300">
                                        {product.price && product.price > 0 ? formatPrice(product.price) : 'Contact for Pricing'}
                                    </p>
                                </div>
                            </div>
                        ))}
                ...
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 md:p-8">
                    ...
                    <div className="relative w-full max-w-6xl bg-neutral-900 rounded-3xl overflow-hidden shadow-2xl border border-white/10 flex flex-col md:flex-row max-h-[90vh] animate-scale-up">
                                ...
                                <div className="p-4 flex gap-2 overflow-x-auto bg-neutral-950/50 backdrop-blur-sm absolute bottom-0 left-0 right-0">
                                    {selectedProduct.images.map((img, idx) => (
                                        <button
                                            ...
                                            className={`relative h-16 w-16 rounded-lg overflow-hidden border-2 transition-all ${activeImageIndex === idx ? 'border-primary-500 scale-105' : 'border-transparent opacity-60 hover:opacity-100'
                                                }`}
                                        >
                        ...
                        <div className="w-full md:w-1/2 p-8 md:p-12 overflow-y-auto custom-scrollbar">
                            <div className="mb-8">
                                <span className="inline-block py-1 px-3 rounded-full bg-neutral-800 text-primary-500 text-xs font-bold tracking-widest uppercase mb-4">
                                ...
                                <p className="text-2xl font-light text-neutral-200">
                                    {selectedProduct.price && selectedProduct.price > 0
                                        ...
                                        : <span className="text-primary-500 font-bold text-lg">Contact for Bulk Pricing</span>
                                    }
                                </p>
                            </div>
                            ...
                            <div className="space-y-8">
                                                        ...
                                                        {values.map((value) => (
                                                            <button
                                                                ...
                                                                className={`px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 border ${selectedVariants[variantType] === value
                                                                    ...
                                                                    : 'bg-neutral-800/50 border-white/10 text-neutral-300 hover:border-primary-500/50 hover:bg-neutral-800'
                                                                    }`}
                                                            >
                                                                {value}
                                                            </button>
                                ...
                                <div className="pt-8 border-t border-white/10 flex flex-col gap-4">
                                    {/* Add to Cart - for products with price */}
                                    {selectedProduct.price && selectedProduct.price > 0 && (
                                        <button
                                            onClick={(e) => selectedProduct && addToCart(e, selectedProduct)}
                                            className="w-full py-4 bg-white text-black rounded-xl font-bold hover:bg-primary-400 transition-colors flex items-center justify-center gap-2 group"
                                        >
                                    ...
                                    <button
                                        onClick={() => {
                                            ...
                                            }, `I'm interested in wholesale pricing for: ${selectedProduct.name}${variantText ? ` (${variantText})` : ''}`);
                                        }}
                                        className="w-full py-4 bg-primary-600 text-white rounded-xl font-bold hover:bg-primary-500 transition-colors flex items-center justify-center gap-2"
                                    >
                                    ...
                                    <p className="text-xs text-center text-neutral-500">
                                        Our sales team will respond within 24 hours with bulk pricing and availability
                                    </p>
                                </div>
                            </div>
                        </div>
